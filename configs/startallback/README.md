...
3. Copy the relevant configuration files into this folder.
4. Run the setup flow so these files are restored on a new machine.

## Notes

StartAllBack may store configuration in a different format depending on the version, so inspect the app data folder and copy only the files that actually contain your preferred settings.
